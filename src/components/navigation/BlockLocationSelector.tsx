...
interface BlockLocationSelectorProps {
  type: 'start' | 'end';
...
export function BlockLocationSelector({
  type,
  selectedNode,
  onSelect,
  placeholder,
}: BlockLocationSelectorProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [query, setQuery] = useState('');
  const inputRef = useRef<HTMLInputElement>(null);
  const dropdownRef = useRef<HTMLDivElement>(null);

  const { filteredLocations, setSearchQuery, searchableLocations } = useBlockNavigation();
