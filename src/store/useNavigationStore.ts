...
import { initialNodes, initialConnections, buildings, rooms } from '@/data/campusData';
...
interface NavigationStore {
  ...
  addNewNode: (x: number, y: number, type: MapNode['type'], name: string, buildingId: string) => void;
...
export const useNavigationStore = create<NavigationStore>((set, get) => ({
  ...
  selectNode: (node) => {
    ...
    if (isConnecting && connectionStart && node && node.id !== connectionStart.id) {
